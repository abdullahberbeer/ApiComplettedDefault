...
namespace KantinAPI.Data.Concrete.EfCore
{
    public class EfOrderRepository:EfCoreGenericRepository<Order>,IOrderRepository
    {
        ...
        {
            var user = KantinContext.Orders.Include(p => p.OrderItems).FirstOrDefaultAsync(x => x.UserId == userId);
            var model = new Order()
            {
                OrderItems = user.Result.OrderItems
            };
        ...
        {
            return await KantinContext.Orders.Include(i => i.OrderItems).ThenInclude(i => i.Product).FirstOrDefaultAsync(i => i.UserId == userId);
        }
